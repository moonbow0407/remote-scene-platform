...
class HealthzResponse(BaseModel):
    ...
    model_config = ConfigDict(title="存活")

    status: str = Field(description="正常时为 ok")


class ReadyzResponse(BaseModel):
    """依赖是否可用。联调前可先看这个。"""

    model_config = ConfigDict(title="就绪")

    status: str = Field(description="全部依赖可用时为 ok")
    components: dict[str, str] = Field(
        description="各依赖状态。键为 db、minio、rabbitmq、titiler，值为 ok 或 unavailable"
    )
...
@router.get(
    "/metrics",
    summary="运行指标",
    description="给监控系统拉取，不是业务接口。前端不要调用。",
)
...
@router.get(
    "/healthz",
    summary="存活检查",
    description="只表示进程还活着，不检查数据库等依赖。联调请再看「就绪检查」。",
...
@router.get(
    "/readyz",
    summary="就绪检查",
    description="检查数据库、对象存储、消息队列、地图服务。有依赖不可用时返回 503。",
