...
def sign_tile_token(
    *, version_id: str, resource: str, ttl_seconds: int, secret: str
) -> tuple[str, int]:
    """签发绑定版本、短期有效的瓦片令牌；返回 (token, 过期Unix秒)。"""
    if not secret:
        raise ProblemError(
            status=503,
    ...
    payload = f"{_TOKEN_PREFIX}.{version_id}.{expires_at}"
    signature = hmac.new(
...
def verify_tile_token(token: str, *, resource: str, secret: str) -> str:
    ...
    _, version_id, expires_raw, signature = parts
    if not secret:
        raise TileTokenError("服务端未配置瓦片令牌密钥", status=503)
    expected = hmac.new(
        secret.encode(),
        f"{_TOKEN_PREFIX}.{version_id}.{expires_raw}.{resource}".encode(),
        hashlib.sha256,
    ).hexdigest()[:32]
